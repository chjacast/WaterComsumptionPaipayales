...
// registro de errores
#define REGISTRO_ERROR 72
#define LONGITUD_ERROR 16
...
//Estructura para almacenar error
union
{
uint16_t a;
float b;
...
void loop() 
{
  ...
  // Imprimimos por pantalla posibles errores
  ...
  float errors = float(error.a)
  
  String color = String(flujoso)+","+String(consumos)+","+String(errors); 
  const char* nuevo="";
...
void leer_error(){
 uint16_t result;
 result = sensor.readHoldingRegisters(REGISTRO_ERROR,LONGITUD_ERROR);
 delay(1500);
 if (result == sensor.ku8MBSuccess)
  {
   error.a=(((unsigned long)sensor.getResponseBuffer(0x00)<<16) | (sensor.getResponseBuffer(0x01)));
   }
 }
...
//  Serial.println("Temperatura 2 es:  "+String(tem.te,5)+" grados Celcius"); 
  ...
  Serial.println("Existe error?:  "+String(error.a));
  }
